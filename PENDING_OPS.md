...
OWNER_ACTIONS:
  ...
  items:
    ...
    - id: stripe-account
      title: "Create Stripe account + products/prices + wire the webhook (web billing CODE is built) — HALF-DONE: Pro test prices set, Career+ NOT (nightly RED); + NEW team seat price STRIPE_PRICE_TEAM_ANNUAL (run 39)"
      team_tier_note: "Run 39 (PR #348) shipped the team/organization SEAT-TIER backend: POST /api/org/checkout makes a REAL quantity-based Stripe Checkout for N seats and refuses honestly (503, no charge) until STRIPE_PRICE_TEAM_ANNUAL is set. To sell team seats: create a PER-SEAT recurring price in Stripe (recommended structure per BUSINESS_CASE lever 2: a self-serve starter seat price + route >50 seats to sales) and set STRIPE_PRICE_TEAM_ANNUAL in the deploy env (and, for CI real-validation, as a GitHub Actions secret). Same webhook endpoint handles org events (they carry an org_id). No new secret-shaped var (STRIPE_PRICE_* is a price id, not a key). The web/mobile team-admin UI is a loop follow-up (ROADMAP Track C) — until it ships, the tier is API-reachable only."
      priority: high
      status: open
    ...
    - id: revenuecat
      ...
      priority: normal
      status: open
      why: "Mobile entitlement verification needs the cross-store keys + product mapping. Both CODE halves are now BUILT: (SERVER, PR #87) POST /api/billing/revenuecat-webhook verifies a shared-secret Authorization header and flips users.tier ONLY from a verified event (grant on purchase/renewal, revoke on EXPIRATION/PAUSED); forged/missing grants nothing (401); unset secret refuses honestly (503). (CLIENT, run 56) mobile/src/services/purchases.ts + paywall.tsx now initiate the native purchase + restore-purchases flow via react-native-purchases v10, identify RevenueCat's appUserID to our User.id on sign-in, and grant entitlement ONLY via the verified webhook (never client-side). Everything stays INERT until the owner-only keys + products exist: with no EXPO_PUBLIC_REVENUECAT_SDK_KEY the client never loads the native SDK and the paywall degrades honestly. The remaining work is all owner-only (keys, product mapping, signed build, the on-device sandbox-charge round-trip)."
      how: "1) Create the RevenueCat project; map the StoreKit + Play Billing products to a 'premium' entitlement (annual + monthly packages in the current offering; a Career+ product later). 2) In the deploy env set REVENUECAT_WEBHOOK_AUTH to a strong random secret (never commit it). 3) In the RevenueCat dashboard add a webhook -> https://<deploy>/api/billing/revenuecat-webhook with that same Authorization header value. 4) Set EXPO_PUBLIC_REVENUECAT_SDK_KEY (the PUBLIC iOS/Android SDK key — public by design, safe in the bundle; NEVER the secret) in the mobile/EAS build env; the client reads it via app.config.ts -> extra.revenuecatSdkKey. 5) The client already sets RevenueCat's appUserID to our User.id (identifyUser in purchases.ts). 6) Do a TestFlight/internal-track signed build and run the sandbox-charge round-trip to close ACCEPTANCE_AUDIT A4/G4. FOLLOW-UPS (loop, code — not owner): handle the TRANSFER event (revoke the source user) and add event-ordering/idempotency (store last-event timestamp) before high-volume go-live; wire an in-app Career+ upgrade package once that product exists."
    - id: app-signing
