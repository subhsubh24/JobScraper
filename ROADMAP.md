...
- [x] **Seat tier — BACKEND** — SHIPPED (run 39, PR #348, 2 Sonnet reviewers + 3 fresh
      ...
      (Career+ stays individual-only). `tests/test_org_billing.py` (31). Live Stripe team Price ID
...
- [x] Test coverage floor defined and met — branch-coverage floor `fail_under=88` in
      `setup.cfg`, enforced by `scripts/preflight.sh ci` (`--cov-fail-under`); measured ~92%
      this run. Ratcheted 65→85 (#293) → 88 (#337); raise toward ~90 as coverage genuinely improves.
