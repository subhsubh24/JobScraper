...
- **Per-PR LLM-liveness config-drift guard (`tests/test_llm_fallback.py`, file-DISJOINT).** The auditor's tests-evals A→A+ gap: real live-model detection is nightly-only, so a deploy that pins/reverts `GEMINI_MODEL`/`GEMINI_FALLBACK_MODELS` to a dead id sails past the per-PR gate (the 2026-07-09 `gemini-2.0-flash` 502 class). A real per-PR live call needs owner CI secrets + a `.github` edit (forbidden), so I built the strongest key-free guard: assert the ENV-RESOLVED chain (`chat_model()` + `_fallback_chat_models()`, complementing the existing DEFAULTS-only tests) contains no known-decommissioned model AND stays a ≥2-deep, duplicate-free chain. Non-vacuous (a `gemini-2.0-flash` override IS detected). Honest scope: catches config REGRESSION per-PR, NOT a brand-new upstream decommission (that stays nightly). 11/11 in the file green, flake8 clean.
