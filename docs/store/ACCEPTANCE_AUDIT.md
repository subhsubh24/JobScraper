...
| G7 | Store listing assets | Icon, feature graphic, screenshots (real files) | FAIL | **Feature graphic DONE** — committed `docs/store/assets/feature-graphic.png` (1024×500, 24-bit RGB PNG, no alpha — meets the CURRENT Play spec; guarded by `tests/test_store_assets.py`), rendered from a reviewable HTML source (`scripts/store/feature_graphic.html`) with a committed alt-text/accessible description (`docs/store/assets/README.md`, 2026 a11y). Still open: **screenshots** (≥2, 16:9 / 9:16 — need a signed native build; not producible on Linux, web captures NOT substituted) and a **bespoke app icon** (`mobile/assets/images/icon.png` is still the Expo template — a brand-aware illustrative mark is owner/designer work; a programmatically generated icon would read as generic-AI slop and fails the DESIGNER QUESTION). FAIL stands until screenshots + a real icon exist. |
